...
\caption[Mapping data units to context trees]{
    ...
    Example of Context Path ($\alpha$) pointing to a data unit containing a
    segmentation:
    ...
    Example of Context Path ($\beta$) pointing to the same data unit: \texttt{/HB130427/left-DL/*.tiff}
}
...
\caption[Schematic depicting NeuronDepot]{
    ...
    of base folders: upload folders (gray, here just one) with read/write
    permissions for project members and multiple data folders (purple, green,
    red) with read-only permissions for project members.
...
\caption[System Architecture]{
    ...
    The architecture underlying NeuronDepot consists of individual layers (A)
    Graphical user interface layer ... (B) Business logic layer ... (C) Persistence
    layer ... (D) File projection layer. Arrows indicate encapsulation, WebApp
    "sees" Logic, but Logic does not see WebApp
}
