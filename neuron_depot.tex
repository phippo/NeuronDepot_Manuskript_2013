...
collaborations with a transparent, automated data flow and shield scientists
from having to learn new tools or a data structuring paradigm that is
introduced here. With minimum overhead -- one-time data assignment from the
originator -- this approach makes experimental and modelling data available
across the collaboration, and cloud-ready, opening up the data to the
advantages in software updates and hardware scalability associated with elastic
cloud computing. We provide a implementation that relies on existing
synchronization services and is usable from all devices via a reactive web
interface. We are motivating our solution by solving the practical
...
replication and re-analysis to test new hypotheses as knowledge progresses.
...
\subsubsection{Data Acquisition at GinJang}
...
\caption[NeuronDepot and its data flow]{
    \emph{NeuronDepot's and its data flow}
    ...
    based on the file system. In order to integrate files into our workflows,
    our system provides two types of base folders: floating folders for upload
    ...
    is correctly related to each other and that all data stays consistent.
...
\caption[Components of NeuronDepot]{
    \emph{Schemtic representation of the different components of NeuronDepot}
    %\pr{This figure will appear in one column within the final layout}
